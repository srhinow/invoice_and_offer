...
(
	...
	(
		...
		(
			'fields'                  => array('title'),
			'format'                  => '%s',
// 			'label_callback'          => array('tl_iao_templates', 'listEntries'),			
		),
...
class tl_iao_templates extends Backend
{
	...
	public function checkPermission()
	{
		...
		{
			unset($GLOBALS['TL_DCA']['tl_iao_templates']['fields']['allowComments']);
		}
		...
		$GLOBALS['TL_DCA']['tl_iao_templates']['list']['sorting']['root'] = $root;

		// Check permissions to add archives
		if (!$this->User->hasAccess('create', 'newp'))
		{
			$GLOBALS['TL_DCA']['tl_iao_templates']['config']['closed'] = true;
		}
		...
		switch ($this->Input->get('act'))
		{
			...
			case 'edit':
				...
				if (!in_array($this->Input->get('id'), $root))
				{
					...
					if (is_array($arrNew['tl_iao_templates']) && in_array($this->Input->get('id'), $arrNew['tl_iao_templates']))
					{
			...
			case 'show':
				...
				{
					$this->log('Not enough permissions to '.$this->Input->get('act').' news archive ID "'.$this->Input->get('id').'"', 'tl_iao_templates checkPermission', TL_ERROR);
			...
			default:
				...
				{
					$this->log('Not enough permissions to '.$this->Input->get('act').' news archives', 'tl_iao_templates checkPermission', TL_ERROR);
					$this->redirect('contao/main.php?act=error');
				}
				break;
		}
