...
class tl_iao_templates_item extends \iao\iaoBackend
{
	...
	public function checkPermission()
	{
		...
		{
			unset($GLOBALS['TL_DCA']['tl_iao_templates_items']['fields']['allowComments']);
		}
		...
		$GLOBALS['TL_DCA']['tl_iao_templates_items']['list']['sorting']['root'] = $root;

		// Check permissions to add archives
		if (!$this->User->hasAccess('create', 'newp'))
		{
			$GLOBALS['TL_DCA']['tl_iao_templates_items']['config']['closed'] = true;
		}
		...
		switch ($this->Input->get('act'))
		{
			...
			case 'edit':
				...
				if (!in_array($this->Input->get('id'), $root))
				{
					...
					if (is_array($arrNew['tl_iao_templates_items']) && in_array($this->Input->get('id'), $arrNew['tl_iao_templates_items']))
					{
			...
			case 'show':
				...
				{
					$this->log('Not enough permissions to '.$this->Input->get('act').' news archive ID "'.$this->Input->get('id').'"', 'tl_iao_templates_items checkPermission', TL_ERROR);
			...
			default:
				...
				{
					$this->log('Not enough permissions to '.$this->Input->get('act').' news archives', 'tl_iao_templates_items checkPermission', TL_ERROR);
					$this->redirect('contao/main.php?act=error');
				}
			break;
		}
